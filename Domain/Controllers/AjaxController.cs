...
namespace Domain.Controllers
{
  ...
  public class AjaxController : Controller
  {
    ...
    public IActionResult AddComment(PostCommentVM model)
    {
      ...
      if (commentService.AddComment(model, UserId))
      {
        SaveSiteLog(SiteLogTableNames.PublicConsultations, SiteLogAction.Edit, model.SourceId, true, "Нов коментар: " + model.Title + "|" + (model.Comment ?? ""));
      }

      return this.StatusCode(201);
    }

    protected void SaveSiteLog(string tableName, int action, int recordId, bool approved, string details)
    {
