...
using Domain.Areas.Admin.Controllers;
...
namespace Domain.Areas.Admin.Controllers
{
    ...
    public class FileCdnController : BaseAdminController
    {
        ...
        public IActionResult UploadFile(int sourceType, string sourceId, string callback = null, string directUploadMethod = null, int stUsed = 0)
        {
            ...
            var libraries = libService.Select(sourceType, null, null).Where(x => x.IsActive);
            ViewBag.libraries = libraries
                .ToSelectList(x => x.Id, x => x.Title, sourceId);

            if (string.IsNullOrEmpty(sourceId) || sourceId == "0")
            {
                sourceId = libraries.FirstOrDefault()?.Id.ToString();
            }
