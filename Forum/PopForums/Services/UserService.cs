...
namespace PopForums.Services
{
    ...
    User CreateUserByID(int UserID, string name, string email, string password, bool isApproved, string ip);
    ...
    public User CreateUserByID(int UserID, string name, string email, string password, bool isApproved, string ip)
    {
      name = _textParsingService.Censor(name);
      var creationDate = DateTime.UtcNow;
      var authorizationKey = Guid.NewGuid();
      var salt = Guid.NewGuid();
      var hashedPassword = password.GetMD5Hash(salt);
      var user = _userRepository.CreateUserByID(UserID, name, email, creationDate, isApproved, hashedPassword, authorizationKey, salt);
      _securityLogService.CreateLogEntry(null, user, ip, String.Empty, SecurityLogType.UserCreated);
      return user;
    }
